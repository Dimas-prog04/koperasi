  ...
  <div class="hp-wrapper">
    ...
    <main class="hp-main">
      ...
      <div class="hp-content">
              ...
              <table class="hp-table">
                ...
                <tbody>
                  <tr v-for="(item, i) in monitoringData" :key="i">
                    ...
                    <td><button @click="openProductDetail('monitoring')" class="hp-btn-icon hp-btn-icon--blue"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button></td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <!-- === 2. VIEW: KARYAWAN === -->
        <div v-else-if="currentView === 'karyawan'" class="hp-view animate-fade">
          <div class="hp-card">
            <div class="hp-card__header">
              <h3 class="hp-card__title"><span class="hp-dot hp-dot--dark"></span> Akun Karyawan</h3>
              <button @click="showKaryawanModal = true" class="hp-btn-add"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></button>
            </div>
            <div class="hp-table-wrap">
              <table class="hp-table">
                <thead><tr><th>USERNAME</th><th>NAMA LENGKAP</th><th>KONTAK</th><th>STATUS</th><th>AKSI</th></tr></thead>
                <tbody>
                  <tr v-for="(item, i) in karyawanData" :key="i">
                    <td><strong>{{ item.username }}</strong></td>
                    <td><strong>{{ item.nama }}</strong></td>
                    <td><strong>{{ item.kontak }}</strong></td>
                    <td><span :class="['hp-badge', item.status === 'Online' ? 'hp-badge--blue' : 'hp-badge--red']">{{ item.status }}</span></td>
                    <td>
              ...
              <table class="hp-table">
                ...
                <tbody>
                  <tr v-for="(item, i) in penggunaData" :key="i">
                    ...
                    <td><strong>{{ item.kontak }}</strong></td>
                    <td><span :class="['hp-badge', item.status === 'Online' ? 'hp-badge--blue' : 'hp-badge--red']">{{ item.status }}</span></td>
                    <td><button @click="showPenggunaModal = true" class="hp-btn-icon hp-btn-icon--blue"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button></td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div v-else-if="currentView === 'supplier'" class="hp-view animate-fade">
          <div class="hp-card">
            <div class="hp-card__header">
              ...
              <button @click="showSupplierModal = true" class="hp-btn-add"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></button>
            </div>
            <div class="hp-table-wrap">
              <table class="hp-table">
        ...
        <div v-else-if="currentView === 'produk'" class="hp-view animate-fade">
          <div class="hp-card mb-4">
            ...
            <button @click="showProdukModal = true" class="hp-btn-full hp-bg-dark-green text-white">
        ...
        <!-- === 9. VIEW: AKTIFITAS === -->
        <div v-else-if="currentView === 'aktifitas'" class="hp-view animate-fade">
          <div class="hp-dashboard-karyawan-grid mb-4">
            <div class="hp-stat-card hp-stat-card--sm" v-for="i in 3" :key="i">
              <div class="hp-stat-card__icon hp-bg-light-green"><svg viewBox="0 0 24 24" fill="none" stroke="#166534" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div>
              <div><h4 class="hp-stat-card__value" style="font-size:16px;">Dashboard Karyawan {{ i }}</h4><p class="hp-stat-card__label" style="display:flex; align-items:center; gap:4px;"><span class="hp-dot hp-dot--blue" style="width:6px;height:6px;"></span> STATUS <strong style="color:#1E3A8A">ONLINE</strong></p></div>
            </div>
            <div class="hp-stat-card hp-stat-card--sm">
              <div class="hp-stat-card__icon hp-bg-light-green"><svg viewBox="0 0 24 24" fill="none" stroke="#166534" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div>
              <div><h4 class="hp-stat-card__value" style="font-size:16px;">Dashboard Karyawan 4</h4><p class="hp-stat-card__label" style="display:flex; align-items:center; gap:4px;"><span class="hp-dot hp-dot--red" style="width:6px;height:6px;"></span> STATUS <span class="hp-badge hp-badge--red" style="padding:2px 6px; font-size:10px;">Offline</span></p></div>
            </div>
          </div>

          <div class="hp-card">
            <div class="hp-card__header"><h3 class="hp-card__title"><span class="hp-dot hp-dot--dark"></span> Aktifitas</h3></div>
            <div class="hp-filter-row mb-4">
              <div class="hp-input-icon" style="flex:1;">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                <input type="text" placeholder="Cari Hash..." class="hp-input" />
              </div>
              <select class="hp-select"><option>Kategori</option></select>
            </div>
            <div class="hp-table-wrap">
              <table class="hp-table">
                <thead><tr><th>JUDUL AKTIFITAS</th><th>HASH</th><th>TANGGAL</th><th>WAKTU PENCATATAN</th><th>AKSI</th></tr></thead>
                <tbody>
                  <tr v-for="(item, i) in aktifitasData" :key="i">
                    <td><strong>{{ item.judul }}</strong></td>
                    <td style="font-size: 13px; color: #4b5563;"><em>{{ item.hash }}</em></td>
                    <td><strong>{{ item.tanggal }}</strong></td>
                    <td><strong>{{ item.waktu }}</strong></td>
                    <td><button @click="showAktifitasModal = true" class="hp-btn-icon hp-btn-icon--blue"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button></td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <!-- === 10. VIEW: CUSTOM ECOMERCE === -->
        <div v-else-if="currentView === 'custom-ecomerce'" class="hp-view animate-fade">
          <div class="hp-card mb-4">
            <div class="hp-card__header"><h3 class="hp-card__title"><span class="hp-dot hp-dot--dark"></span> Kategori Produk</h3></div>
            <div class="hp-input-row mb-4">
              <input type="text" placeholder="Masukan Nama Produk..." class="hp-input" style="flex:1;" />
              <input type="text" placeholder="Masukan Nomor..." class="hp-input" style="flex:1;" />
              <button class="hp-btn hp-bg-blue text-white"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> TAMBAH</button>
            </div>
            <div class="hp-table-wrap">
              <table class="hp-table">
                <thead><tr><th>NOMOR</th><th>NAMA PRODUK</th><th>AKSI</th></tr></thead>
                <tbody>
                  <tr v-for="i in 5" :key="i">
                    <td><strong>01</strong></td>
                    <td><strong>Mie Sedap Goreng</strong></td>
                    <td>
                      <div class="hp-actions">
                        <!-- Pemicu Pop Up Edit -->
                        <button @click="openEdit" class="hp-btn-icon hp-btn-icon--yellow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
          ...
          <div class="hp-card">
            <div class="hp-card__header"><h3 class="hp-card__title"><span class="hp-dot hp-dot--dark"></span> Banner</h3></div>
            <div class="hp-input-row mb-4">
              <input type="text" placeholder="Masukan Nama Produk..." class="hp-input" style="flex:1;" />
              <input type="text" placeholder="Masukan Nomor..." class="hp-input" style="flex:1;" />
              <button class="hp-btn hp-bg-blue text-white"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> TAMBAH</button>
            </div>
            <button class="hp-btn-full hp-bg-dark-green text-white mb-4" style="border-radius: 8px;">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
              Masukan Gambar
            </button>
            <div class="hp-table-wrap">
              <table class="hp-table">
                <thead><tr><th>NOMOR</th><th>NAMA PRODUK</th><th>GAMBAR</th><th>AKSI</th></tr></thead>
                <tbody>
                  <tr v-for="i in 6" :key="i">
                    <td><strong>01</strong></td>
                    <td><strong>Mie Sedap Goreng</strong></td>
                    <td><a href="#" style="color: #3B82F6; font-weight: 700; text-decoration: none;">Mie_Sedap.jpg</a></td>
                    <td>
                      <div class="hp-actions">
                        <button @click="openProductDetail('banner')" class="hp-btn-icon hp-btn-icon--blue"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
                        <!-- Pemicu Pop Up Edit -->
                        <button @click="openEdit" class="hp-btn-icon hp-btn-icon--yellow"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
                        <!-- Pemicu Pop Up Hapus -->
                        <button @click="openDelete" class="hp-btn-icon hp-btn-icon--red"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
    ...
    <!-- 1. MODAL DETAIL PRODUK (Monitoring & Banner) -->
            ...
            <div class="hp-modal-box">
              ...
              <div class="hp-grid-2">
                ...
                <div class="hp-form-group">
                  ...
                  <input type="text" placeholder="Contoh : ujang kedu.." class="hp-input" />
                </div>
              </div>
    ...
    <!-- 3. MODAL PENGGUNA (Detail Read Only) -->
    <Transition name="hp-fade">
        ...
        <div class="hp-modal hp-modal--scrollable" style="max-width: 800px;" @click.stop>
          ...
          <div class="hp-modal__body">
            ...
            <div class="hp-modal-box mb-4">
              ...
              <div class="hp-form-group" style="margin-top: 16px;">
                <label>USER AGENT (BROWSER/OS)</label>
                <div class="hp-input-mock">CHROME,2342343, LINUX</div>
              </div>
            </div>
              ...
              <div class="hp-json-tables">
                <table class="hp-json-table">
                  ...
                  <tbody>
                    ...
                    <tr><td>updated_at</td><td>2026-05-23T03:31:45.000000Z</td></tr>
                  </tbody>
                </table>
                <table class="hp-json-table">
                  ...
                  <tbody>
                    ...
                    <tr><td>updated_at</td><td>2026-05-23 06:12:33</td></tr>
                  </tbody>
                </table>
              </div>
            </div>
            ...
            <div class="hp-modal-box">
              ...
              <div class="hp-form-group">
                ...
                <div class="hp-input-mock" style="letter-spacing: 1px;">768789654I83YFHVB J6R5E</div>
              </div>
            </div>
...
const detailModalType = ref('monitoring') // 'monitoring' atau 'banner'
...
// State Modal Edit & Hapus Universal
const showEditModal = ref(false)
const showDeleteModal = ref(false)

// === FUNGSI PEMICU MODAL (Bisa digunakan di seluruh tombol kuning dan merah) ===
